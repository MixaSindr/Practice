...
int main() {
    ...
    //for (int i = centr_h-size; i <= centr_h + size; ++i) {
    //    for (int j = centr_w - size; j <= centr_w + size; ++j) {
    //        ggg.write((char*)&c, sizeof(c));     //запись сразу всего цвета
    //    }
    //}
    if (centr_h - size<1 || centr_h + size>h || centr_w - size<1 || centr_w + size>w) {
        cout << "invalid size";
        return 0;
    }
    for (int i = 1; i <= h; ++i) {
        ...
        for (int j = 1; j <= w; ++j) {
            if ((i > centr_h - size) && (i < centr_h + size) && (j > centr_w - size) && (j < centr_w + size)) {
